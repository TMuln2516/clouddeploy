			...
			<div class="search-wrapper" style="margin-top: -5rem;">
				...
				<ul class="nav nav-tabs search-nav-tabs" id="myTab" role="tablist">
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="<c:url value="/?loggedin=user"/>">TRANG CHỦ</a></li>
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="#">VIỆC LÀM</a></li>
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="./aboutus">VỀ CHÚNG TÔI</a></li>
							...
							<div class="row">
								<div class="col-md-10 col-sm-12">
									<div class="input-group s-input-group">
										...
										<input type="text" class="form-control sinput" name="search" value="${search }"
						...
						<c:choose>
							<c:when test="${not empty listJobs }">
									...
									<div class="job-group">
										<div class="job pagi">
											<div class="job-content">
												...
												<div class="job-desc">
													<div class="job-title">
														<a href="<c:url value="/detail?jobID=${job.job_id }&username=${name}"/>">${job.title}</a>
													</div>
													<div class="job-company">
														<a >${job.company_name }</a> | <a 
															class="job-address"><i class="fa fa-map-marker"
															aria-hidden="true"></i> ${job.city}</a>
													</div>
													
													<div class="job-inf">
														<div class="job-salary">
															<i class="fa fa-money" aria-hidden="true"></i> <span
																class="salary-min"><fmt:formatNumber value="${job.min_salary}" pattern="#,###"></fmt:formatNumber></span>
															<span class="salary-max"><fmt:formatNumber value="${job.max_salary}" pattern="#,###"></fmt:formatNumber></span>
														</div><br/>
								...
								<c:if test="${currentPage > 1 }">
									<a
										href="<c:url value="search?currentPage=${currentPage - 1}"/>">Previous</a>
								</c:if>

								<c:if test="${startPage > 2 }">
									<a href="<c:url value="search?currentPage=1"/>">1</a>
									<span>...</span>
								</c:if>
								...
								<c:forEach var="pageNumber" begin="${startPage }"
									...
									<c:choose>
										...
										<c:otherwise>
											<a
												href="<c:url value="search?currentPage=${pageNumber}"/>">${pageNumber}</a>
										</c:otherwise>
									</c:choose>
								</c:forEach>
								...
								<c:if test="${endPage < totalPages}">
									<span>...</span>
									<a
										href="<c:url value="search?currentPage=${totalPages }"/>">${totalPages }</a>
								</c:if>

								<c:if test="${currentPage < totalPages }">
									<a
										href="<c:url value="search?currentPage=${currentPage + 1}"/>">Next</a>
								</c:if>
							</c:when>
	...
	<footer class="container-fluid copyright-wrap">
		<div class="container copyright">
			<p class="copyright-content">
				Copyright © 2023 <a href="#"> FindJob</a>. All Right Reserved.
			</p>
		</div>
	</footer>
